...
var prefixNumber = String.fromCharCode(0x283C);

function isUpper(ch) {
    return ch.toUpperCase() === ch;
}

function wordMatchingRegex() {
...
function findLongestUpperCaseSequence(text) {
    ...
    // iterate through character sequence until it's non-uppercase
...
function findLongestNumericSequence(text) {
    ...
    //iterate through input until a terminator character appears
    for (var i = 0; i < text.length; i++) {
        var ch = text.charAt(i);
        var isTerminator = (isWhitespace(ch) || isLetter(ch));
        if (isTerminator) {
...
function unicodeToBrailleSimple(text) {
    var result = '';
    for (var i = 0; i < text.length; i++) {
...
function unicodeToBraille(text) {
    ...
    while (text.length > 0) {
        ...
        if (upperCaseSequence.length > 0) {
            ...
            //emit individual letters
            result += unicodeToBrailleSimple(upperCaseSequence.toLowerCase());
        ...
        if (numericSequence.length > 0) {
            ...
            //emit individual numbers and symbols
            result += unicodeToBrailleSimple(numericSequence);

            //look ahead if we continue with a letter
            var nextChar = peek(text);
            if (nextChar != null) {
                if (isCharacter(nextChar)) {
                    //emit end of prefix - continuing with a letter
                    result += prefixEnd;
                }
            }
